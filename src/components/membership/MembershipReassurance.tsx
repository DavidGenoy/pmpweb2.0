...
export default function MembershipReassurance({ items, className = "" }: { items: readonly string[]; className?: string }) {
  return (
    <ul className={`flex flex-wrap justify-center gap-x-5 gap-y-2 text-sm text-white/70 ${className}`}>
      {items.map((item) => (
        <li key={item} className="flex items-center gap-1.5 whitespace-nowrap">
          <Check aria-hidden="true" className="h-4 w-4 shrink-0 text-accent-400" />
          {item}
        </li>
      ))}
    </ul>
